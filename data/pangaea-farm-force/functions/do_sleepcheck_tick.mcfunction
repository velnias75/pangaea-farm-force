...
execute if score #bett schlafzeit matches 11615 run execute as @a[scores={sleeping=0},nbt={Dimension:"minecraft:overworld"}] run tellraw @s ["",{"text":"[Server] "},{"selector":"@p"},{"text":", es wird allmählich "},{"text":"Nacht","bold":true,"color":"blue"},{"text":". Bitte suche das "},{"text":"nächste Bett","bold":true,"color":"gold"},{"text":" auf."}]
